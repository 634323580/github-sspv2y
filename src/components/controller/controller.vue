  ...
  <transition name="controller">
    <div class="play-controller" v-if="song.title" @click="songShow()">
      <div class="song_info">
        <div
          class="song_cover"
          :class="{ rotatePlay: true, rotatePaused: !playState }"
        >
          <img
            width="35"
            height="35"
            :src="
              song.pic_small
                ? song.pic_small
                : ' /static/res/ccnn/0e2442a7d933c8953ee45510d21373f0830200c7.jpg'
            "
            alt=""
          />
        </div>
      ...
      <div class="play-controller-btn">
        <div class="play-btn" @click.stop="playGo()">
          ...
          <i
            ...
            :class="{ playIcon: !playState }"
            v-html="playState ? '&#xe600;' : '&#xe624;'"
          ></i>
        </div>
        <div @click.stop="playList()" class="play-list-btn iconfont">
          &#xe926;
        </div>
      </div>
    </div>
  ...
  methods: {
    playGo() {
      ...
      this.$store.commit('setPlayState', {
        state: !this.$store.state.playState,
      });
    },
    songShow() {
      this.$emit('songShow');
